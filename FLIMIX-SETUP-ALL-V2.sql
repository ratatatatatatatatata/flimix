-- ============================================================
-- FLIMIX — БҮРЭН SQL SETUP (v2, дубляжийн дуутай)
-- Дахин ажиллуулахад аюулгүй: public schema-г цэвэрлээд эхнээс барина.
-- auth.users дахь бүртгэлүүд ХЭВЭЭР үлдэнэ, профайлууд сэргээгдэнэ.
-- Анхаар: контентын дата (кино, төрөл г.м) устаж дахин үүснэ.
-- ============================================================
...
-- 3) PostgREST хандах эрх (Supabase default)
grant all on all tables in schema public to anon, authenticated, service_role;
grant all on all functions in schema public to anon, authenticated, service_role;
grant all on all sequences in schema public to anon, authenticated, service_role;

-- 4) Лавлах өгөгдөл: улс, төрөл, хэл
insert into public.countries (code, name_mn, name_en) values
  ('MN', 'Монгол', 'Mongolia'),
  ('KR', 'Солонгос', 'South Korea'),
  ('CN', 'Хятад', 'China'),
  ('US', 'Америк', 'United States')
on conflict do nothing;

insert into public.genres (slug, name_mn, name_en) values
  ('action', 'Адал явдалт', 'Action'),
  ('comedy', 'Инээдмийн', 'Comedy'),
  ('drama', 'Драм', 'Drama'),
  ('romance', 'Хайр дурлалын', 'Romance'),
  ('thriller', 'Триллер', 'Thriller'),
  ('horror', 'Аймшгийн', 'Horror'),
  ('fantasy', 'Уран зөгнөлт', 'Fantasy'),
  ('sci-fi', 'Шинжлэх ухааны', 'Science Fiction'),
  ('animation', 'Хүүхэлдэйн', 'Animation'),
  ('documentary', 'Баримтат', 'Documentary'),
  ('family', 'Гэр бүлийн', 'Family'),
  ('crime', 'Гэмт хэргийн', 'Crime')
on conflict do nothing;

insert into public.languages (code, name_mn, name_en) values
  ('mn', 'Монгол', 'Mongolian'),
  ('en', 'Англи', 'English'),
  ('ko', 'Солонгос', 'Korean'),
  ('zh', 'Хятад', 'Chinese')
...
-- 5) Бүртгэлтэй хэрэглэгчдийн профайл + эрх сэргээх
insert into public.profiles (user_id, display_name)
...
from auth.users
where email = 'turuu.naranbaatar0711@gmail.com'
on conflict do nothing;

-- 7) Шалгалт
select 'tables' as what, count(*)::text as value
from information_schema.tables where table_schema = 'public'
union all
select 'admin_granted', count(*)::text
from public.user_roles ur
join auth.users u on u.id = ur.user_id
where u.email = 'turuu.naranbaatar0711@gmail.com' and ur.role = 'super_admin'
union all
select 'genres', count(*)::text from public.genres
union all
select 'audio_url_column', count(*)::text
from information_schema.columns
where table_name = 'audio_tracks' and column_name = 'url';
